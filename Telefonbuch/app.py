...
def uebersicht():
    ...
    return render_template('uebersicht.html', personen=personen)

# Datenbank füllen
@app.cli.command('seed')
def seed_db():
    beispieldaten = [
        Person(name='Max Mustermann', adresse='Musterstraße 1, 12345 Musterstadt', telefonnummer='01234-56789'),
        Person(name='Erika Muster', adresse='Beispielweg 2, 54321 Beispielstadt', telefonnummer='09876-54321'),
        Person(name='John Doe', adresse='Unknown Street 99, 99999 Nowhere', telefonnummer='11111-22222')
    ]
...
@app.cli.command('import_sqlite_personen')
def import_sqlite_personen():
    import sqlite3
    # Pfad zur alten SQLite-Datei
    sqlite_conn = sqlite3.connect('telefonbuch.db')
    cursor = sqlite_conn.cursor()

    # Daten abrufen
    cursor.execute("SELECT name, adresse, telefonnummer FROM person")
    daten = cursor.fetchall()

    # In MySQL einfügen
    for name, adresse, telefonnummer in daten:
        neue_person = Person(name=name, adresse=adresse, telefonnummer=telefonnummer)
        db.session.add(neue_person)

    db.session.commit()
    sqlite_conn.close()
    print("Personen erfolgreich aus SQLite übernommen.")
